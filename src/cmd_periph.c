...
int cmd_mag_setgain(struct command_context *ctx) {
	...
	mag_set_conf(mag_get_rate(), mag_get_meas(), (mag_gain_t)gain);
	return CMD_ERROR_NONE;
}
...
int cmd_mag_setrate(struct command_context *ctx) {
	...
	mag_set_conf((mag_rate_t)rate, mag_get_meas(), mag_get_gain());
	return CMD_ERROR_NONE;
}
...
struct command mon_commands[] = {
	{
		...
		.name = "loopraw",
		.help = "Magnetometer raw read in a loop",
		.handler = mag_test_raw,
	},{
		.name = "status",
...
struct command accel_sensor_i2c_commands[] = {
        {
                .name = "read",
                .help = "accel_sensor_i2c_test",
                .handler = accel_sensor_i2c_test,
	},
};
struct command camera_usart_commands[] = {
        {
                .name = "test",
                .help = "camera_usart_commands",
...
struct command __root_command periph_commands[] = {
	{
		...
		.name = "accel_sensor_i2c",
		.help = "accel_sensor_i2c commands",
		.chain = INIT_CHAIN(accel_sensor_i2c_commands),
	},{
                .name = "camera_usart",
                .help = "camera_usart commands",
                .chain = INIT_CHAIN(camera_usart_commands),
}
};
