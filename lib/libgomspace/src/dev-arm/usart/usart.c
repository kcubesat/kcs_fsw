...
#define USART_RX_QUEUE_LENGTH 	100
#define USART_RX_BUF_LENGTH		0x1000
#define USART_TX_BUF_LENGTH 	0x1000

#define PICTURE_SIZE		4803		//add sungkeun
//#define portTIME ( portTickType ) 0x0000
...
static void __attribute__((noinline)) usart_DSR(int handle) {
	...
	static unsigned portLONG status;

	xTaskWoken = pdFALSE;
	status = usart[handle].base->SR;
...
void usart_init(int handle, uint32_t fcpu, uint32_t usart_baud) {
	...
	if (usart[handle].usart_rxqueue == NULL)
		...
		if (handle == 1)							// add sungkeun
			usart[handle].usart_rxqueue = xQueueCreate(PICUTER_SIZE, 1);	// add sungkeun
...
//	xQueueReceive(usart[handle].usart_rxqueue, &buf, portMAX_DELAY);
	xQueueReceive(usart[handle].usart_rxqueue, &buf, 100);
	return buf;
}
